    ...
    <div className={dark ? 'portfolio dark' : 'portfolio light'}>
      ...
      <main id="main-content">
        ...
        <section id="work" className="section-shell section-block"><div className="section-heading"><div><span className="section-index">01</span><h2>Projects</h2></div><p>AI automation systems that move ideas<br />from interesting to impactful.</p></div>
          <div className="projects-grid">{projects.map((project, index) => <motion.article key={project.title} className={`project-card ${index === 0 ? 'project-featured' : ''}`} initial={{ opacity: 0, y: reduceMotion ? 0 : 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.5, delay: index * 0.08 }} whileHover={reduceMotion ? {} : { y: -8 }}><a className="project-link" href={project.href || undefined} target={project.href ? '_blank' : undefined} rel={project.href ? 'noreferrer' : undefined} aria-label={project.href ? `View ${project.title} on GitHub` : undefined}><div className={`project-art bg-gradient-to-br ${project.accent}`}><span className="project-number">{project.number}</span><Sparkles className="project-spark" aria-hidden="true" /><div className="art-lines" /></div><div className="project-body"><div className="project-meta"><span>{project.type}</span><span className="project-arrow">↗</span></div><h3>{project.title}</h3><p>{project.description}</p><div className="tag-row">{project.tags.map(tag => <span key={tag}>{tag}</span>)}</div></div></a></motion.article>)}</div>
        </section>
